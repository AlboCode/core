...
class AgentManager:
    ...
    def execute_agent(self, working_memory):
        ...
        if len(fast_reply.keys()) > 0:
            return fast_reply
        prompt_prefix = mad_hatter.execute_hook("agent_prompt_prefix", prompts.MAIN_PROMPT_PREFIX, working_memory.get('user_message_json'))
        prompt_suffix = mad_hatter.execute_hook("agent_prompt_suffix", prompts.MAIN_PROMPT_SUFFIX, working_memory.get('user_message_json'))
